...
class DynamicModulePlugin : Plugin<Settings> {
    ...
    override fun apply(target: Settings) {
        target.run {
            ...
            val type = TypeToken.getParameterized(List::class.java,
                TsGrammar::class.java)
            val grammars: List<TsGrammar> =
                Gson().fromJson(grammarsFile.bufferedReader(), type.type)

            for (grammar in grammars) {
                val moduleDir =
                    rootDir.resolve("grammar-modules/${grammar.name}")
                        .also { it.mkdirs() }
    ...
    private fun writeCmakeLists(grammar: TsGrammar, cppDir: File, rootDir: File
        ...
        file.writeText(
            grammar.cmakeListsSrc(rootDir).autoGenerated(comment = "##"))
    }
...
Java_com_itsaky_androidide_treesitter_${name}_TSLanguage${capitalizedName()}_00024Native_newInstance(JNIEnv *env,
                                                                                  jclass clazz) {
